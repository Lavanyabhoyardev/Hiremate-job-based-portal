...
const getJobSuggestions = asyncHandler(async (req, res) => {
  ...
  // Update cache only if we actually found jobs (prevents caching empty error states)
  if (ranked.length > 0) {
    const cacheEntry = {
      timestamp: Date.now(),
      ranked,
    ...
    // If we fetched 0 jobs due to random rate-limit or fail, clear the cache to allow retry
